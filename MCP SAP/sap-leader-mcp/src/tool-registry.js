// tool-registry.js — daftar tool tunggal dipakai bersama oleh stdio (index.js) dan HTTP (http-server.js)

import * as serverTools from './tools/server-tools.js';
import * as queryTools from './tools/query-tools.js';
import * as abapTools from './tools/abap-tools.js';
...
export const TOOLS = [
  ...
  },
  {
    ...
    description:
      'Cek tanggal server SAP saat ini dengan membaca TRDAT terbesar dari tabel USR02 (last logon date). Gunakan ini sebelum input tanggal di server yang tanggalnya tidak sesuai kalender nyata (mis. Sandbox New Company TRS).',
    inputSchema: { type: 'object', properties: {} },
...
export function registerToolHandlers(server, { CallToolRequestSchema, ListToolsRequestSchema }) {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      return {
        content: [
          { type: 'text', text: JSON.stringify({ error: err.message, stack: err.stack, tool: name }, null, 2) }
        ],
        isError: true
      };
    }
